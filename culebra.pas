...
var
        row,col:byte;
begin
        for row:=2 to 24 do begin
                if (row = 2) or (row = 24) then begin
                        for col:=2 to 79 do begin
                                textbackground(blue);
                                gotoxy(col,row);
                                write(' ');
                        end;
...
begin
        tamanoculebra:=6;
        for tculebra:=1 to tamanoculebra do begin
...
begin
        if first then
                ...
                tculebra:=2;
        helper:=false;
        while (tculebra<=tamanoculebra) and not helper do begin
...
begin
        ...
        for tculebra:=tamanoculebra downto 2 do begin
        ...
        if (posculebra[1][1] = poscomida[1]) and (posculebra[1][2] = poscomida[2]) then begin
                puntos:=puntos+10;
                puntos_es;
                if tamanoculebra<=50 then begin
        ...
        if (posculebra[1][1] = 2) or (posculebra[1][1] = 79) or (posculebra[1][2] = 2) or (posculebra[1][2] = 24) then begin
                jugando:=false;
        end;
        if en_arreglo(posculebra[1][1], posculebra[1][2], false) then begin
                jugando:=false;
        end;
end;
